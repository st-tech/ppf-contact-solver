...
cd /d "%SRC_DIR%"
REM This host has the CUDA toolkit, so build.rs blocks emulated builds by
REM default (they would clobber the real CUDA binary). The rig wants the stub
REM here, so opt in explicitly.
set "PPF_ALLOW_EMULATED=1"
