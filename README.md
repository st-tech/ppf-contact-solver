...
- This README is mostly hand-typed and then proofread by an LLM with minimal changes. Minor parts (e.g., tables) are greatly assisted by an LLM.
- Python docstrings are mostly auto-generated by an LLM.
...
- The paper is first drafted entirely in English in the author's own voice. An LLM is used to proofread and polish the wording, though this iterative process has introduced minor expansions or contractions, all of which are carefully human-checked and corrected where necessary. The author understands all the contents.
- The paper draft is written directly in English, **not in Japanese and then translated**. Writing directly in English avoids hallucinated nuances that translation can introduce. The author has verified not only surface-level meaning but also delicate nuances throughout. Ultimately, the author understands the fine-grained details.
