...
impl std::hash::Hash for FaceParam {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        ...
        self.offset.to_bits().hash(state);
        self.bend.to_bits().hash(state);
        self.strain_limit_tau.to_bits().hash(state);
        self.strain_limit_eps.to_bits().hash(state);
...
impl PartialEq for FaceParam {
    fn eq(&self, other: &Self) -> bool {
        self.model == other.model
            ...
            && self.offset.to_bits() == other.offset.to_bits()
            && self.bend.to_bits() == other.bend.to_bits()
            && self.strain_limit_tau.to_bits() == other.strain_limit_tau.to_bits()
            && self.strain_limit_eps.to_bits() == other.strain_limit_eps.to_bits()
