    ...
    pub fn make_props(&self, mesh: &MeshSet, face_area: &[f32], tet_volume: &[f32]) -> Props {
        ...
        let face = (0..mesh.mesh.mesh.face.ncols())
            .map(|i| {
                ...
                let mut bend = None;
                let mut shrink = None;
                let mut strain_limit_tau = None;
                let mut strain_limit_eps = None;
                let mut ghat = None;
                let mut offset = None;
                let mut friction = None;
                let mut density = None;
                for (name, value) in &self.tri_param {
                    ...
                    } else if name == "strain-limit-tau" {
                        strain_limit_tau = match value {
                            ParamValueList::Value(v) => Some(v[i]),
                            _ => panic!("Expected strainlimit tau parameter to be a value list"),
                        };
                    } else if name == "strain-limit-eps" {
                        strain_limit_eps = match value {
                            ParamValueList::Value(v) => Some(v[i]),
                            _ => panic!("Expected strainlimit eps parameter to be a value list"),
                        };
                    } else if name == "bend" {
                        bend = match value {
                            ParamValueList::Value(v) => Some(v[i]),
                ...
                let strain_limit_tau = strain_limit_tau.unwrap();
                let strain_limit_eps = strain_limit_eps.unwrap();
                let ghat = ghat.unwrap();
                let offset = offset.unwrap();
                let friction = friction.unwrap();
                let density = density.unwrap();
            ...
            let face_props = (0..self.static_tri.ncols())
                .map(|i| {
                    ...
                    let param = FaceParam {
                        ...
                        offset,
                        bend: 0.0,
                        strain_limit_tau: 0.0,
                        strain_limit_eps: 0.0,
                        shrink: 1.0,
                    };
                    let param_idx = *face_param_map.entry(param).or_insert_with(|| {
                        let new_idx = face_params.len() as u32;
