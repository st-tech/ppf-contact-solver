...
__device__ Vec2f point_edge_distance_coeff(const Vec3f &p, const Vec3f &e0,
                                           const Vec3f &e1) {
    Vec3f r = e1 - e0;
    float d = r.squaredNorm();
    if (d > EPSILON) {
        float x = r.dot(p - e0) / d;
...
__device__ Vec3f point_triangle_distance_coeff(const Vec3f &p, const Vec3f &t0,
                                               const Vec3f &t1,
                                               const Vec3f &t2) {
    Vec3f r0 = t1 - t0;
    Vec3f r1 = t2 - t0;
    ...
    Vec2f c;
    if (!solve(a_t * a, a_t * (p - t0), c)) {
        c = Vec2f(1.0f / 3.0f, 1.0f / 3.0f);
    }
    return Vec3f(1.0f - c[0] - c[1], c[0], c[1]);
}

__device__ Vec4f edge_edge_distance_coeff(const Vec3f &ea0, const Vec3f &ea1,
                                          const Vec3f &eb0, const Vec3f &eb1) {
    Vec3f r0 = ea1 - ea0;
    Vec3f r1 = eb1 - eb0;
    Mat3x2f a;
    a << r0, -r1;
    Eigen::Transpose<Mat3x2f> a_t = a.transpose();
    Vec2f x;
    if (solve(a.transpose() * a, a.transpose() * (eb0 - ea0), x)) {
...
__device__ Vec3f point_triangle_distance_coeff_unclassified(const Vec3f &p,
                                                            const Vec3f &t0,
                                                            const Vec3f &t1,
                                                            const Vec3f &t2) {
...
__device__ float point_triangle_distance_squared_unclassified(const Vec3f &p,
                                                              const Vec3f &t0,
                                                              const Vec3f &t1,
                                                              const Vec3f &t2) {
    Vec3f c = point_triangle_distance_coeff_unclassified(p, t0, t1, t2);
    Vec3f x = c(0) * t0 + c(1) * t1 + c(2) * t2;
    return (p - x).squaredNorm();
}

__device__ Vec4f edge_edge_distance_coeff_unclassified(const Vec3f &ea0,
                                                       const Vec3f &ea1,
                                                       const Vec3f &eb0,
                                                       const Vec3f &eb1) {
...
__device__ float edge_edge_distance_squared_unclassified(const Vec3f &ea0,
                                                         const Vec3f &ea1,
                                                         const Vec3f &eb0,
                                                         const Vec3f &eb1) {
