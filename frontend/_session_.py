    ...
    def start(
        ...
        if self._cmd_path:
            ...
            if fixed_scene is not None:
                tau_vals = [
                    x
                    for x in fixed_scene.tri_param.get("strain-limit-tau", [])
                    if isinstance(x, float)
                ]
                eps_vals = [
                    x
                    for x in fixed_scene.tri_param.get("strain-limit-eps", [])
                    if isinstance(x, float)
                ]
                if tau_vals and eps_vals:
                    max_strain_limit = max([t + e for t, e in zip(tau_vals, eps_vals)])
