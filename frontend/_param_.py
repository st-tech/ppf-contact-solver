...
def app_param() -> dict[str, tuple[Any, str, str]]:
    ...
    return {
        ...
        "strain-limit-reduction": (
            1e-4,
            "Strain Limit Reduction",
            "This factor is used to reduce the strain limit over time.",
        ),
        "binary-search-max-iter": (
            1024,
            "Maximum Iterations for Binary Search",
            "The maximum number of iterations for binary search in strain limiting line search.",
        ),
        "toi-reduction": (
            0.9,
            "TOI Reduction Rate",
            "This parameter defines the rate at which the Time of Impact (TOI) is reduced when the strain limiting line search does not settle.",
        ),
    }
...
def object_param(obj_type: str) -> dict[str, tuple[Any, str, str]]:
    ...
    return {
        ...
        "strain-limit-tau": (
            0.0,
            "Strain Limit Tau",
            "Starting from this value, the strain limit is activated. This can be zero",
        ),
        "strain-limit-eps": (
            0.0,
            "Strain Limit Epsilon",
            "After the strain limit is activated, this value is used to control the maximal stretch. When set to zero, it deactivates strain limiting.",
        ),
    }
...
class ParamHolder:
    ...
    def set(self, key: str, value: Any) -> "ParamHolder":
        if key == "strain-limit":
            # Special handling: set both strain-limit-eps and strain-limit-tau
            self.set("strain-limit-eps", 0.5 * value)
            self.set("strain-limit-tau", 0.5 * value)
