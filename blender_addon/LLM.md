...
- When the full docs site is built separately, it uses a `docs/.venv` environment.
...
- **Python API** via `from bl_ext.user_default.ppf_contact_solver.ops.api import solver`: the
  ...
  `solver.create_curve(...)`, `group.create_pin(...)`, pin operation
  chaining, collider builders (`Wall`, `Sphere`), `solver.snap(...)`,
  `solver.clear()`, `solver.transfer_data() / run_simulation() / fetch_animation() / update_params()`.
- Fallback operator dispatch: any `bpy.ops.zozo_contact_solver.<tool>`
  is callable when the Python API doesn't cover a surface.
- Pointer to the bundled per-tool and per-method references
  (`mcp_tools_reference.md` and `python_api_reference.md` in this LLM
  tree, also exposed over MCP as `llm://mcp_tools_reference` and
  `llm://python_api_reference`).
...
- You need to distinguish two similarly-named tools (e.g.
  `connect_ssh` vs `connect_docker`, `set_group_material_properties`
  vs `set_scene_parameters`).
...
`SceneParam`, `DynParam`, `Group`, `GroupParam`, `Pin`, `Curve`,
`Wall`, `Sphere`, `ColliderParam`. Each entry has the typed signature,
...
- **Skim the headings first.** Every file has predictable
  ...
  the source docs.
- **Full reference docs are bundled here:** `mcp_tools_reference.md`
  and `python_api_reference.md` are the MCP-served reference sources
  that ship with the add-on. Load one of these when you need an exact
  method or tool signature.
