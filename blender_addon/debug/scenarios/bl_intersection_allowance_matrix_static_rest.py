# File: scenarios/bl_intersection_allowance_matrix_static_rest.py
# Code: Claude Code
# Review: Ryoichi Ando (ryoichi.ando@zozo.com)
# License: Apache v2.0
#
# The intersection allowance matrix, HELD object a REST-POSE STATIC collider:
# a 1 m grid lying flat at z = 0 in a STATIC group with no animation, under a
# SHELL, SOLID, ROD, SAND or PDRD object dropped from 0.1 m above it, with an
# invisible floor at z = -0.3.
#
# A rest-pose STATIC decodes to a contact-only collision mesh that carries NO
# policy of its own, so its own checkboxes reach nothing, while a dynamic
# object flagged inter-object or inter-group passes through it. For each
# falling type (see `_allowance_matrix.pair_cells`):
#
#   a  no flag                              -> held
#   b  inter-object on the falling group    -> passes
#   c  inter-object on the STATIC group     -> held (reaches nothing)
#   d  inter-group on the falling group     -> passes
#   e  inter-group on the STATIC group      -> held (reaches nothing)
#   f  self on both groups                  -> held (the wrong flag)
#
# There is no g cell: a STATIC group carries Transform operations, not pins,
# so there is no pin item to flag.
#
...
CELLS = am.pair_cells("STATIC_REST")
