# File: scenarios/bl_elastic_drape.py
# Code: Claude Code
# Review: Ryoichi Ando (ryoichi.ando@zozo.com)
# License: Apache v2.0
#
# Exercises the elastic solve end-to-end through the production pipeline:
# a sheet pinned along one edge and released under gravity DRAPES, free
# vertices sagging while the pinned edge holds. Both halves are the point.
# A run where nothing moved would pass a frame-count check alone, and a run
# where the pinned edge moved too would mean the constraint was not applied,
# so the scenario builds that scene, runs the solver, fetches the per-frame
# PC2, and asserts:
#
...
try:
    ...
    dh.save_blend(PROBE_DIR, "elastic_drape.blend")
    root = dh.configure_state(
        project_name="elastic_drape",
    ...
    # negative). With the solver this would be ~0.
