...
WARNING: All objects must not intersect and must not be self-intersecting at the start of the simulation. The solver rejects rest geometry where any face penetrates another face (on the same object or on a different one), and it cannot recover from a start state that is already inside out. Before Transfer, confirm that every mesh is cleanly separated from every other mesh and that no mesh folds through itself. This rule applies to every object type that has faces: Solid, Shell, Rod, PDRD, and Static. A Sand body has no faces, so the face test does not apply to it; what it needs instead is that no two grain centers start within one grain diameter of each other, which the Convert To Solid Particle Mesh seeding guarantees.
...
- Sand groups take a mesh that has been run through **Convert To Solid Particle Mesh**: a faceless cloud of grain-center vertices. Assign the pre-conversion solid mesh to the group, then convert it in place.
...
Generated or imported geometry is worth scanning before Transfer rather than after a rejected build: **Sidebar -> Utility Tools -> Mesh Cleaning -> Scan Selected Meshes**, the `scan_meshes` MCP tool, and `solver.scan_meshes(...)` from Python all report the near-coincident vertices, loose vertices, degenerate faces, duplicate faces, and inconsistent winding that stop a run, each with a dedicated repair. See "Mesh will not build" in troubleshooting.md for the error-to-tool mapping.
...
| **Sand**   | Granular bodies (sand, grain, gravel) | N/A          | N/A                                        |
| **Static** | Non-deforming collision objects   | N/A              | N/A                                        |

The type controls which material parameters are relevant and which material models are available. Static groups collapse to Friction, Apply Soft Constraints, and the Contact rows, and replace the pin region with a Transform sub-box that holds per-object Move By / Spin / Scale ops (an alternative to Blender transform keyframes). See Static Objects for the full surface.
...
Sand groups simulate a granular body: a faceless mesh whose vertices are grain centers, each treated as a sphere of one shared radius that collides and rubs against its neighbors and against everything else in the scene. There is no material model and no elasticity, so a Sand group carries only a grain radius, a per-grain mass, an inter-grain friction, and a contact gap (see Material Parameters -> Sand-specific).

A Sand group's input is not modeled by hand. Assign an ordinary closed solid mesh whose volume is the shape you want filled, select it, and press **Convert To Solid Particle Mesh** in the group box (directly above **Delete Group**). The operator replaces the mesh with a Poisson-disk cloud of grain centers filling that volume and attaches a render-only Particle Mesh modifier so the grains draw as spheres. It is destructive: the original faces are discarded, so keep an unconverted copy if you may want to re-seed. The grain count is not chosen; it is whatever fits at the chosen radius and spacing, and it is reported when the operation finishes. The radius is stamped on the object and locked afterward, because the non-overlapping seed spacing derives from it and it is also the grain's contact skin.
...
Figure: Reference matrix of the types. Shell (green), Solid (red), Rod (yellow), PDRD (magenta), Sand (tan), Static (blue). Accepted object types: Shell/Solid/PDRD/Static take meshes; Rod takes meshes plus Bezier curves; Sand takes a mesh that has been run through Convert To Solid Particle Mesh. Default material model: Baraff-Witkin for Shell, ARAP for Solid and Rod, Rigid for PDRD, none for Sand or Static. Available models: Shell offers Baraff-Witkin / Stable NeoHookean / ARAP; Solid offers Stable NeoHookean and ARAP; Rod offers ARAP only; PDRD offers the rigid model only; Sand and Static none. Density unit: kg/m² for Shell, kg/m³ for Solid and PDRD, kg/m for Rod; Sand instead carries a per-grain mass in grams. Young's Modulus: Shell/Solid/Rod, not PDRD, Sand or Static. Poisson's Ratio: Shell and Solid. Bend Stiffness: Shell and Rod, writing the same `bend` field but scaled per type (a rod's is normalized against a 1 cm reference segment, so it does not vary with segment count). Shrink: Shell anisotropic X/Y, Solid uniform, Rod a rest-length scale (`length_factor`), PDRD/Sand/Static none. Strain Limit: Shell and Rod. Inflate: Shell only. Rayleigh deformation damping: Shell/Solid/Rod; bending damping: Shell and Rod only; PDRD and Sand not damped. Friction and Contact Gap: all types; Sand takes its contact offset from the locked grain radius rather than from a field. Pin storage: Blender vertex groups for Shell/Solid/PDRD/Sand, internal `_pin_name` custom property on curves for Rod, none for Static (uses a Transform sub-box). Default overlay colors: green, red, yellow, magenta, tan, blue. Static is the thinnest column because the solver normally only uses it for collision; no material model, and no parameters beyond Friction, Contact Gap, and Apply Soft Constraints (which holds the collider with springs so contact can push it off its animated path, at the cost of simulating it rather than only colliding against it). The Material Params box reshapes itself automatically to match the column.
...
Deformation capture internals. The Capture Deformation operator scrubs the Blender depsgraph from the solve's starting frame (`resolve_start_frame()`, see Time conversion below) to the highest keyframe across every Action that influences the object (the object's own animation_data, its shape-keys animation_data, every parent in the chain, and every modifier's `.object` target). Driver-only and constraint-only setups, which expose no Actions, fall back to `scene.frame_end`. Decoupling from `scene.frame_end` means a 90-frame bone action with the timeline extended to 180 produces a 90-frame cache that releases the collider at frame 91 (so the cloth can fall freely afterward), rather than a 180-frame cache that freezes the collider at the last posed frame. The cache is stored as a UUID-keyed PC2 sidecar under `<blend_dir>/data/<blend_basename>/<uuid>_staticdeform.pc2`; `clear_static_deformation` deletes it.

Time conversion. Frame values in the UI and MCP handlers are Blender frames. The encoder converts to seconds as `(frame - start) / fps`, using the effective FPS from `resolve_fps()` (the Blender scene's frame rate when `use_scene_fps` is on, otherwise the add-on's `frame_rate` field) and the effective starting frame from `resolve_start_frame()` (the scene's start frame when `use_scene_frame_start` is on, otherwise the add-on's `frame_start` field; see Scene Parameters). The starting frame IS simulated time zero, so it defines where the solve sits on the timeline: with the default of 1 the conversion is the historical `(frame - 1) / fps`. Both resolvers follow the same rule: the plain field keeps its last value while its scene override is on, so anything reporting a rate or a start frame must report the resolved value, never the raw field.
