...
- **STATIC-specific**: `enable_soft_constraint`, `soft_constraint_stiffness`. A collider tracks its animation exactly by default and no contact force can move it. Enabling soft constraints holds each of its vertices with a spring of `soft_constraint_stiffness` toward that animated position instead, so the collider yields where the contact pushes harder than the spring. Use it when the collider's own geometry closes onto cloth (an armpit shutting, a hand pressing into a thigh) and traps it with nowhere to go. The stiffness must be greater than zero, and a Static group carrying it is simulated rather than merely collided against, even when it never moves.
