...
"""
    ...
    steps:
      ...
      - name: Build + install ppf-cts-py wheel into remote venv
        # Mirror blender.yml's maturin step: build the PyO3 extension
        # `_ppf_cts_py` and install it editable into the remote venv at
        # ~/.local/share/ppf-cts/venv. Without this the frontend
        # dispatchers fail at import time because frontend/__init__.py
        # hard-imports `_ppf_cts_py`. warmup.py creates the venv but
        # does not build the wheel; cargo build alone only builds the
        # workspace target/ binaries, not the Python extension.
        run: |
          echo "Building and installing ppf-cts-py wheel on remote..."
          INSTANCE_ID=$(cat /tmp/instance_id.txt)
