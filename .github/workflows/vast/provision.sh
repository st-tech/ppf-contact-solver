...
while true; do
  ...
  # write collect command to a file
  echo "rsync -avz --exclude='bin' --exclude='*.bin' -e \"ssh -i $WORKDIR/id_ed25519 -o StrictHostKeyChecking=no -p $port\" root@${hostname}:/tmp/ci ." >$WORKDIR/collect.sh
