...
// The rotation bodies the wrappers below call, as a plain host compiler reads
// them: the `cpp` rendering of the neutral kernel, produced by
// ppf-cts-compute/seam/kernelgen.py and named from the generated root that -I supplies.
// pdrd_polar.hpp names no rendering itself, because nvcc compiles it too and
// reads a differently named one, so the includer supplies the body.
// THE VOCABULARY FIRST, because a kernel body carries none. `pdrd_rigid`
// names `VertexProp` and `compute::atomic_float_t` and includes
// `csrmat/fixed_csr.kernel.cpp`, and all of them reach their spellings from
// whichever shared header the INCLUDER pulled in ahead of them. This is the
// SAME first include `entrypoints/entries.cpp` makes, and for the same reason:
// `data.hpp` opens with `seam/seam.hpp`, the one file that picks the backend
// prologue, so `compute::` exists before any body names it. `data_records.hpp`
// alone is not enough: it carries the records but not the seam, and the
// failure then reads as a defect in the body (a member of "non-class type
// const int", `compute` "has not been declared") rather than as a missing
// prologue. GCC quotes those lines from the NEUTRAL file, because the
// rendering opens with `#line` naming it, which is what makes the diagnostic
// point at code that is fine.
#include "data.hpp"

#include "energy/model/pdrd_rigid.kernel.cpp"
...
int g_failures = 0;
// `PI` IS A MACRO IN `common.hpp`, which the vocabulary include above now
// brings in, so a local constant of that name is a redefinition rather
// than a shadow. The test keeps its own spelling instead of undefining
// the project one, which would leave the next includer of this file
// without it.
...
float angle_deg(const float R[9]) {
    ...
    if (t > 1.0f) t = 1.0f;
    if (t < -1.0f) t = -1.0f;
    return std::acos(t) * 180.0f / TEST_PI;
}
...
int main() {
    ...
    std::printf("[2] exact 180-deg rotations (the bug case)\n");
    {
        ...
        for (int a = 0; a < 4; ++a) {
            float Rt[9], M[9], R[9];
            axis_angle(axes[a][0], axes[a][1], axes[a][2], TEST_PI, Rt);
            cube_M(Rt, M);
            rigid_polar_quat(M, R);
