...
// License: Apache v2.0
//
// Host regression for shell_bend_directional
// (energy/model/shell_bend_stiffness.kernel.cpp), which combines a shell
// hinge's isotropic bending stiffness with the two directional ones according
// to where its shared edge sits in the UV material frame:
//
...
// The `cpp` rendering of the neutral kernel, produced by ppf-cts-compute/seam/kernelgen.py
// and named from the generated root that -I supplies.
// THE VOCABULARY FIRST, as `test_pdrd_polar.cpp` and `entrypoints/entries.cpp`
// both do, and for the same reason: a kernel body carries none.
// `shell_bend_stiffness` names `VertexProp` (in
// `shell_bend_areal_density_from_records`), and a rendering compiled with the
// body as its first include parses that as a type it has never seen. `data.hpp`
// opens with `seam/seam.hpp`, so the seam prologue arrives with the records.
#include "data.hpp"
...
void anchors_pin_the_sign_convention() {
    ...
    expect_near("edge along warp (sin2=0) -> bend + weft",
                shell_bend_directional(bend, warp, weft, 0.0f), bend + weft,
                1e-6f);
    expect_near("edge along weft (sin2=1) -> bend + warp",
                shell_bend_directional(bend, warp, weft, 1.0f), bend + warp,
                1e-6f);
    ...
    expect_near("edge at 45 degrees -> bend + (warp + weft)/2",
                shell_bend_directional(bend, warp, weft, 0.5f),
...
void isotropic_defaults_are_bit_exact() {
    ...
    for (int i = 0; i <= 100000; ++i) {
        ...
        if (shell_bend_directional(7.5f, 0.0f, 0.0f, s) != 7.5f) {
            ++off;
        }
    }
...
void no_uv_sentinel_is_isotropic() {
    expect_true("sentinel -> bend exactly",
                shell_bend_directional(2.25f, 11.0f, 5.0f, -1.0f) == 2.25f);
}
        ...
        for (int wi = 0; wi <= 40; ++wi) {
            ...
            for (int fi = 0; fi <= 40; ++fi) {
                ...
                for (int i = 0; i <= 100; ++i) {
                    ...
                    if (shell_bend_directional(bend, warp, weft, s) < 0.0f) {
                        ++neg;
                    }
                }
            }
...
void stiffness_stays_between_the_axes() {
    ...
    for (int i = 0; i <= 10000; ++i) {
        ...
        const float k = shell_bend_directional(bend, warp, weft, s);
...
void direction_only_so_scale_free() {
    ...
    for (int i = 1; i < 200; ++i) {
        ...
        const float base = shell_bend_directional(bend, warp, weft, s);
        for (float k : {0.001f, 0.5f, 3.0f, 1000.0f, -1.0f}) {
            ...
            expect_near("UV scale / edge reversal leaves the stiffness alone",
                        shell_bend_directional(bend, warp, weft, s2), base,
                        1e-4f);
        }
    }
}
