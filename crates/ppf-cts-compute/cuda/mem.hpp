...
// kernel bodies: `cudaMemcpy` is a mechanism, and a tree whose contents belong
...
#include "cuda_utils.hpp" // CUDA_HANDLE_ERROR
#include "cuda_vec.hpp"   // ArenaPtr<T>, ArenaVec<T>
...
// blocking copy that usually follows it. `cudaMemcpy` on the default stream
...
inline void wait_for_device() { CUDA_HANDLE_ERROR(cudaDeviceSynchronize()); }
