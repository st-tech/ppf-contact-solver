...
// `cuda_vec.hpp` reads them under `#ifdef __CUDA_ARCH__` and reads
// `g_host_bases` and `g_host_arena_count` on the host branch. Do not give
// either of these a host reader.
//
