...
// gets a pointer into the same pages through `cudaHostGetDevicePointer`, so no
...
// THE NULL CHECK IS WHAT KEEPS A FIRING ASSERT A REPORT RATHER THAN A CRASH.
//
// `bind` yields `header` from `channel.device_base`, which is null until the
// channel is created, so a kernel handed a channel nothing created gets a null
// header. Drop the guard and the FIRST statement on a failing condition is
// `atomicAdd(&header->fail_count, 1u)`, which is `atomicAdd((unsigned *)0,
// 1u)`: a null device write, reported by the next synchronization as
// `cudaErrorIllegalAddress`. The violated invariant never reaches the host, and
// what the reader gets instead is a platform fault naming no cause.
//
// THE COST OF THAT IS PAID ONLY WHEN THE REPORT MATTERS. A run in which nothing
// asserts touches these pages from the device zero times, so the defect is
// invisible until a check fires, which is the moment the report was needed.
// The host spelling in `kernels/seam/seam_host.h` tests its pointer for the
// same reason; this is that check on the device side.
//
// It is NOT a substitute for attaching a real channel. A guarded assert with no
// channel is SILENT, and silence is its own defect: `be_create` wires the
// backend's channel in as the global one so these records reach `drain_diag`.
// This guard is what keeps a missing channel from being fatal, not a license to
// run without one.
