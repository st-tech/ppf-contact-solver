...
// `fatal`. Included outside the __CUDACC__ guard below because it needs no
// CUDA runtime header, so a host-only compile of this header still gets them.
...
// pre-allocated / high-water pools have warmed up. Plain host globals, which
// is what lets every backend link them. Defined once in
// crates/ppf-cts-compute/cuda/backend/backend.cu.
extern unsigned long long g_device_alloc_count;
extern unsigned long long g_device_free_count;

// A compile with no `__CUDACC__`, and so no cuda_runtime.h, takes the stub arm
// below. The macro's replacement list does not name its parameter, so a
// caller's argument (the cudaMalloc and cudaFree calls in arena/arena.cu) is
// discarded as preprocessor tokens instead of being compiled. Nothing in this
// tree takes that arm: every file that includes this header is a `.cu`, the
// generated `*.entry.cu` among them, or one of the two nvcc-only headers beside
// it, cuda_vec.hpp and mem.hpp.
#ifdef __CUDACC__
#include <cstdio>
#include <cstdlib>
...
// by the preflight in `be_open`. Negative until that runs. A crash report
...
static void HandleError(cudaError_t err, const char *file, int line) {
    if (err != cudaSuccess) {
        ...
        if (code == FATAL_WATCHDOG_TIMEOUT && used > 0 &&
            (size_t)used < sizeof(detail)) {
...
// nvcc defines these as keywords; a host-only compile gets
...
#define __device__
#endif
// `cudaStream_t` is declared by <cuda_runtime.h>, which only the __CUDACC__ arm
// above includes. This stand-in lets a compiler that is not nvcc parse the
// header. Every translation unit that reaches this file is a `.cu`, so no build
// selects this arm.
